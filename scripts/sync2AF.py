...
def sync2freqh(x):
    ''' convert string in SYNC format to dictionary of frequencies where x is a string in sync format'''
    nuc = ["A", "T", "C", "G"]
    counts = list(map(int, x.split(":")[:4]))
...
def load_data(x):
    ...
    if x == "-":
        ...
        y = gzip.open(x, "rt")  # 'rt' mode for reading text in Python 3
    else:
        y = open(x, "r")
    return y
...
freqh = d(list)
o1 = open(f"{options.out}_pos.txt", "w")
o2 = open(f"{options.out}_freq.txt", "w")
